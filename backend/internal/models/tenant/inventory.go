...
type InventoryItem struct {
	...
	Name           string         `gorm:"not null" json:"name"`
	SKU            string         `gorm:"uniqueIndex" json:"sku"` // Should be validated for uniqueness in application logic.
...
type InventoryTransaction struct {
	...
	TransactionType string     `json:"transaction_type"` // stock_in, stock_out. Validate in application logic.
	Quantity        int        `gorm:"not null" json:"quantity"`
	UnitPrice       float64    `json:"unit_price"`
	VendorID        *uuid.UUID `gorm:"type:uuid;index" json:"vendor_id"` // Indexed for performance.
...
type DeviceAssignment struct {
	...
	SerialNumber   string     `json:"-"` // Sensitive, do not expose in JSON. Validate in application logic.
	MacAddress     string     `json:"-"` // Sensitive, do not expose in JSON. Validate in application logic.
